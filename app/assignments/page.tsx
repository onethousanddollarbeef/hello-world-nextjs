        ...
        <main className="mx-auto flex min-h-screen w-full max-w-4xl flex-col gap-6 px-6 py-16">
            <header className="flex flex-wrap items-center justify-between gap-3">
                ...
                <div className="flex items-center gap-2">
                    <Link className="rounded-lg border border-pink-500 bg-pink-600 px-4 py-2 text-sm font-medium text-white transition hover:bg-pink-500" href="/">
                        Home
                    </Link>
                    <Link className="rounded-lg border border-pink-500 bg-pink-600 px-4 py-2 text-sm font-medium text-white transition hover:bg-pink-500" href="/project1">
                        Back to Project 1
                    </Link>
                    {user ? (
                        <form action={handleSignOut}>
                            <button className="rounded-lg border border-pink-500 bg-pink-600 px-4 py-2 text-sm font-medium text-white transition hover:bg-pink-500" type="submit">
                                Sign out
                            </button>
                        </form>
                    ) : null}
            ...
            <section className="rounded-2xl border border-zinc-200 bg-white p-6 shadow-sm dark:border-zinc-700 dark:bg-zinc-900">
                ...
                <ul className="space-y-3">
                    {assignments.map((item) => (
                        <li key={item.href}>
                            <Link
                                className="block rounded-lg border border-pink-500 bg-pink-600 px-4 py-3 text-sm font-medium text-white transition hover:bg-pink-500"
                                href={item.href}
                            >
                                {item.label}
                            </Link>
