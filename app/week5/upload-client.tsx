...
export default function Week5UploadClient() {
    ...
    const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
        ...
        try {
            ...
            if (currentFileKey && uploadedFileKey === currentFileKey && imageId) {
                setCurrentStep(4);
                const captionRequest = await requestCaptions(imageId, authHeaders);
                const captionsResponse = "response" in captionRequest ? captionRequest.response : captionRequest;

                if (!captionsResponse.ok) {
                    const body = "errorBody" in captionRequest ? captionRequest.errorBody : await captionsResponse.text();
                    setStatus("idle");
                    setCurrentStep(0);
                ...
                let normalized = normalizeCaptionPayload(captionsPayload);

                if (normalized.length < 5) {
                    const retryRequest = await requestCaptions(imageId, authHeaders);
                    const retryResponse = "response" in retryRequest ? retryRequest.response : retryRequest;
                    if (retryResponse.ok) {
                        const retryPayload = await retryResponse.json();
                        normalized = [...normalized, ...normalizeCaptionPayload(retryPayload)];
                    }
                }
                ...
                while (finalCaptions.length > 0 && finalCaptions.length < 5) {
                    finalCaptions.push({
                        id: `pending-${finalCaptions.length + 1}`,
                        content: "More captions are generating. Run again for additional options.",
                    });
                }
            ...
            const captionRequest = await requestCaptions(registerPayload.imageId, authHeaders);
            const captionsResponse = "response" in captionRequest ? captionRequest.response : captionRequest;
            ...
            if (normalized.length < 5) {
                const retryRequest = await requestCaptions(registerPayload.imageId, authHeaders);
                const retryResponse = "response" in retryRequest ? retryRequest.response : retryRequest;
                if (retryResponse.ok) {
                    const retryPayload = await retryResponse.json();
                    normalized = [...normalized, ...normalizeCaptionPayload(retryPayload)];
    ...
    return (
        <section className="rounded-2xl border border-zinc-700 bg-zinc-900 p-6 shadow-sm">
            ...
            {imageUrl && captions.length > 0 && status === "done" ? (
            ...
            {imageId ? (
                <p className="mt-1 break-all text-xs text-zinc-400">Image ID: {imageId}</p>
            ) : null}
